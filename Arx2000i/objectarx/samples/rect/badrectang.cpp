...
void registerForDemandLoading () {
    //----- Get the Running AutoCAD Registry entry
	const char *pProductKey =acrxProductKey () ;
	//----- Register the Application in the AutoCAD registry (demand loading)
	acrxRegisterApp (
		(AcadApp::LoadReasons)(AcadApp::kOnCommandInvocation | AcadApp::kOnLoadRequest),
		/*NOXLATE*/"Rectang", pProductKey, 2
	) ;
	//----- Now register ourself
    char pKey [MAX_PATH] ;
    sprintf (pKey, /*NOXLATE*/"%s\\AutodeskApps\\Rectang", pProductKey) ;
	HKEY rkey ;
	DWORD result ;
	LONG status =RegCreateKeyEx (HKEY_LOCAL_MACHINE, pKey, 0, "", REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &rkey, &result) ;
	if ( status != ERROR_SUCCESS )
		return ;
	//----- Group Name
	HKEY groupKey ;
	status =RegCreateKeyEx (rkey, /*NOXLATE*/"Groups", 0, "", REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &groupKey, &result) ;
	if ( status == ERROR_SUCCESS ) {
		RegSetValueEx (groupKey, /*NOXLATE*/"ACAD_RECTANGLE", 0, REG_SZ, (const unsigned char *)/*NOXLATE*/"ACAD_RECTANGLE", 15) ;
		RegCloseKey (groupKey) ;
	}
	//----- Commands
	HKEY cmdKey ;
	status =RegCreateKeyEx (rkey, /*NOXLATE*/"Commands", 0, "", REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &cmdKey, &result) ;
	if ( status == ERROR_SUCCESS ) {
		RegSetValueEx (cmdKey, /*NOXLATE*/"RECTANG", 0, REG_SZ, (const unsigned char *)"RECTANG", 8) ;
		RegSetValueEx (cmdKey, /*NOXLATE*/"RECTANGLE", 0, REG_SZ, (const unsigned char *)"RECTANGLE", 10) ;
		RegCloseKey (cmdKey) ;
	}
	//----- Loader
	HKEY loaderKey ;
	status =RegCreateKeyEx (rkey, /*NOXLATE*/"Loader", 0, "", REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &loaderKey, &result) ;
	if ( status == ERROR_SUCCESS ) {
	    char *module =acedGetAppName () ;
		RegSetValueEx (loaderKey, /*NOXLATE*/"MODULE", 0, REG_SZ, (const unsigned char *)module, strlen (module) + 1) ;
		RegCloseKey (loaderKey) ;
	}
	//----- Register Name
	HKEY nameKey ;
	status =RegCreateKeyEx (rkey, /*NOXLATE*/"Name", 0, "", REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &nameKey, &result) ;
    if ( status == ERROR_SUCCESS ) {
	    RegSetValueEx (nameKey, /*NOXLATE*/"Rectang", 0, REG_SZ, (const unsigned char *)/*NOXLATE*/"Rectang", 8) ;
	    RegCloseKey (nameKey) ;
    }
    //----- Closing main key
	RegCloseKey (rkey) ;
