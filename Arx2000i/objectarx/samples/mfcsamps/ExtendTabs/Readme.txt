...
Some key points need to know:
	- this sample uses MFC extension dll;
	- # derivatives are provided in stdafx.h which enables your debug
	  project to work with release Acad;
	- there is a new temporary resource override class;
	- _hdllInstance is a global variable

This application adds two "Tabs" to the "Options" dialog box in AutoCAD 2000.
...
our ObjectARX application and the AutoCAD 2000 dialog that we wish to extend.

The "EXTTABS" command is pretty straight forward in that we launch our modal
...
dialog (AddTab).  The "OPTIONS" dialog is a resizable dialog. You will see in
this section of the application we respond to the resizing event, using the
following functions:
...
The tabbed dialogs themselves are pretty straight forward. In the CTab1 class,
...
The function that we override are:

CAdUiTabChildDialog::OnMainDialogOK(),
CAdUiTabChildDialog::OnMainDialogHelp(),
CAdUiTabChildDialog::OnMainDialogOK()

there are many others available to you also.
