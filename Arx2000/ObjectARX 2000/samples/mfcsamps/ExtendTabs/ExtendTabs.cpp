...
void initApp()
{
	acedRegCmds->addCommand("RX_CMDS", "EXTTABS", "EXTENDTABS", 
		ACRX_CMD_MODAL, &ExtendTabsCreate);

	// Express interest in adding tabs to the OPTIONS dialog.
	// The name of the OPTIONS dialog in T058 and after is "OptionsDialog".
...
// Add the tabs here.
{
	...
	gpTab1->StretchControlXY(IDC_EDIT1, 50, 50);
	gpTab1->StretchControlXY(IDC_EDIT2, 50, 50);
    gpTab1->MoveControlX(IDC_EDIT2, 100);
	gpTab1->MoveControlX(IDC_STATIC2, 100);
	pXtabManager->AddTab(_hdllInstance, IDD_TAB2,"My Tab2", gpTab2);
	gpTab2->StretchControlXY(IDC_LIST1, 50, 100);
	gpTab2->MoveControlX(IDC_RADIO1, 100);
	gpTab2->MoveControlX(IDC_RADIO2, 100);
	gpTab2->MoveControlX(IDC_RADIO3, 100);
	gpTab2->MoveControlX(IDC_STATIC1, 100);
	gpTab2->StretchControlXY(IDC_LIST2, 50, 100);
	gpTab2->MoveControlX(IDC_LIST2, 100);
	gpTab2->MoveControlY(IDC_STATIC3, 100);
	gpTab2->MoveControlY(IDC_EDIT1, 100);
	gpTab2->MoveControlY(IDC_BUTTON, 100);
}
