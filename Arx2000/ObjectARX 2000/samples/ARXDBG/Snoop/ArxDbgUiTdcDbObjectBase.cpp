...
ArxDbgUiTdcDbObjectBase::display(AcDbMText* mtext)
{
    ...
    addToDataList(_T("Is R12 Style Text"), ArxDbgUtils::booleanToStr(mtext->isR12StyleText(), str));

    addToDataList(_T("Is Def Alignment"), ArxDbgUtils::booleanToStr(mtext->isDefaultAlignment(), str));
    if (mtext->isDefaultAlignment() == Adesk::kFalse)
        addToDataList(_T("Alignment Pt"), ArxDbgUtils::ptToStr(mtext->alignmentPoint(), str));

    addToDataList(_T("Thickness"), ArxDbgUtils::doubleToStr(mtext->thickness(), str));
    addToDataList(_T("Oblique Angle"), ArxDbgUtils::angleToStr(mtext->oblique(), str));
    addToDataList(_T("Is Mirrored In X"), ArxDbgUtils::booleanToStr(mtext->isMirroredInX(), str));
    addToDataList(_T("Is Mirrored In Y"), ArxDbgUtils::booleanToStr(mtext->isMirroredInY(), str));
    addToDataList(_T("Horizontal Mode"), ArxDbgUtils::textModeToStr(mtext->horizontalMode(), str));
    addToDataList(_T("Vertical Mode"), ArxDbgUtils::textModeToStr(mtext->verticalMode(), str));
...
void
ArxDbgUiTdcDbObjectBase::display(AcDbRegion* region)
{
    CString str;
    ...
    es = region->getPerimeter(perimeter);
    if (es == Acad::eOk)
        addToDataList(_T("Perimeter"), ArxDbgUtils::doubleToStr(perimeter, str));

    es = region->getArea(area);
    if (es == Acad::eOk)
        addToDataList(_T("Area"), ArxDbgUtils::doubleToStr(area, str));

    es = region->getAreaProp(origin, xAxis, yAxis, perimeter, area, centroid, momInertia,
                prodInertia, prinMoments,
