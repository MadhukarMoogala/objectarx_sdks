...
AcDbDictionary*
getDict( const char *dictName, AcDb::OpenMode mode, AcDbDatabase *pDb )
{
...
AcDbObjectId
getDictId( const char *dictName, AcDbDatabase *pDb )
{
    if (NULL == pDb) pDb = acdbHostApplicationServices()->workingDatabase();
...
AcDbObjectId
getExtDictId( AcDbObject *pObj, const char *dictName )
{
    if (NULL == pObj)
        return AcDbObjectId( 0 );
