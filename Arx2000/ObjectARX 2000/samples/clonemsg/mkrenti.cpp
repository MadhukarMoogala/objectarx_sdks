...
AsdkMkrEntity::list() const
{
    assertReadEnabled();
    AcDbEntity::list();
    acutPrintf( "%18sCenter at: %-9.16q0,%-9.16q0,%-9.16q0\n",
        "", m_center[0], m_center[1], m_center[2] );
    acutPrintf( "%18sAssociated object id: %x\n", "", m_refId.asOldId() );
}
...
AsdkMkrEntity::dxfInFields(AcDbDxfFiler* filer)
{
  ...
  if ((AcDbEntity::dxfInFields(filer) != Acad::eOk) ||
      !filer->atSubclassData(/*NOXLATE*/"AsdkMkrEntity")) {
...
AsdkMkrEntity::dxfOutFields(AcDbDxfFiler* filer) const
{
  ...
  es = filer->writeItem(AcDb::kDxfSubclass, /*NOXLATE*/"AsdkMkrEntity");
  assert(es == Acad::eOk);
