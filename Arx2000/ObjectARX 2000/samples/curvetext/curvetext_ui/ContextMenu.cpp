...
curvetextContextMenu::onCommand(Adesk::UInt32 cmdIndex)
{
	...
	case ID_CURVETEXT_REVERSE:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_reverse ");
		break;

	case ID_CURVETEXT_CURVE_SHOW:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_showCurve ");
		break;

	case ID_CURVETEXT_CURVE_HIDE:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_hideCurve ");
		break;

	case ID_CURVETEXT_TEXT_SHOW:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_ShowText ");
		break;

	case ID_CURVETEXT_TEXT_HIDE:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_HideText ");
		break;

	case ID_CURVETEXT_FITTEXT_YES:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_fit ");
		break;

	case ID_CURVETEXT_FITTEXT_NO:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_noFit ");
		break;

	case ID_CURVETEXT_REPEATTEXT_YES:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_repeatText ");
		break;

	case ID_CURVETEXT_REPEATTEXT_NO:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_noTextRepetition ");
		break;

	case ID_CURVETEXT_SETTEXTSTARTPOINT:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_ssp ");
		break;

	case ID_CURVETEXT_SETTEXTENDPOINT:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_sep ");
		break;
	}
}
...
curveContextMenu::onCommand(Adesk::UInt32 cmdIndex)
{
	...
	case ID_CURVES_CURVETEXT:
		acDocManager->sendStringToExecute(curDoc(), "ADCG_ct ");
		break;
	}
}
