...
#include "dbents.h"
#include "acestext.h"
#include "lab8CommandUtils.h"

#include <stdio.h>

#include "..\object\lab8ObjectEmployeedetails.h"
#include "..\object\lab8ObjectEmployeeentry.h"
...
{ 
    ...
    try
    {
        ...
        ADSOK(acedGetString(0,"Enter first employee name:",strFirstName));

        char strLastName[133];
        ADSOK(acedGetString(0,"Enter last employee name:",strLastName));
        ...
        //add the persistent reactor
        pO->addPersistentReactor(pEmployeeDetails->objectId());
    ...
    case AcRx::kInitAppMsg:
        {
            ...
            //check for the service registered by Lab8Object
            AcRxObject* pSvc;
            if (!(pSvc = acrxServiceDictionary->at(EMPLOYEE_DETAILS_SERVICE))){
                //try to load lab8Object if it is not yet present
                if (!acrxDynamicLinker->loadModule("lab8object.dbx", 0)){
                    acutPrintf("Unable to load Lab8Object.dbx. Unloading this application...\n");
                    return AcRx::kRetError;
                } 
            }
            if (!(pSvc = acrxServiceDictionary->at(EMPLOYEE_ENTRY_SERVICE))){
                acutPrintf("Service % is not present. This must be a wrong version of lab8Object.\n",EMPLOYEE_ENTRY_SERVICE);
                return AcRx::kRetError;
            }
            //register the command
            acedRegCmds->addCommand("OARX_LAB8","addemp","addemp",ACRX_CMD_MODAL,addEmpCommand);
            acedRegCmds->addCommand("OARX_LAB8","removeemp","removeemp",ACRX_CMD_MODAL,removeEmpCommand);
            acedRegCmds->addCommand("OARX_LAB8","listemps","listemps",ACRX_CMD_MODAL,listEmpsCommand);
            break;
        }
    case AcRx::kUnloadAppMsg:
        {
			acrxDynamicLinker->unloadModule("lab8object.dbx" ) ;
            //remove the command group so that AutoCAD won't call us
            //when we are not here.
            acedRegCmds->removeGroup("OARX_LAB8");
            break;
        }
    }
    return AcRx::kRetOK;
