...
{
	TRACE("CAcStCircle::CAcStCircle() called.\n");
...
{
	TRACE("CAcStCircle::FinalConstruct() called.\n");
	return S_OK;
}
...
{
	TRACE("CAcStCircle::FinalRelease() called.\n");
	Clear();
}
...
{
    TRACE("CAcStCircle::CheckSysvar() called. Not Implemented!\n");
	return E_NOTIMPL;
}
...
{
	TRACE("CAcStCircle::Initialize() called.\n");
...
CAcStCircle::Clear()
{
	TRACE("CAcStCircle::Clear() called.\n");
	...
	{	
		...
		for(int i=0; i<m_objIdsFix.length(); i++)
		{
			...
			if( es == Acad::eOk && pCir != NULL )
			{
				...
				createNewFix(&pFix, CString("Color fix"), cirCache.standardFileName);
				CString str;
				char strHandle[20];
				m_objIdsFix[i].handle().getIntoAsciiBuffer(strHandle);
				str.Format("Color index %d of Circle %s", cirCache.color, strHandle);
...
{
	TRACE("CAcStCircle::StampDatabase called.\n");
...
{
	TRACE("CAcStCircle::WritePluginInfo called.\n");
...
{
	TRACE("CAcStCircle::Start() called.\n");
	...
	while (m_curIndex < m_objIdsToCheck.length())
	{
		...
		if ( bFoundError )
       {
           ...
           m_pError = NULL;
			...
			str.Format("color index %d of Circle %s", colorIndex, strHandle); 
...
{
	TRACE("CAcStCircle::GetError() called.\n");
...
{
	TRACE("CAcStCircle::SetContext() called.\n");
...
{
	TRACE("CAcStCircle::GetObjectFilter() called.\n");
...
{
	TRACE("CAcStCircle::GetRecommendedFix() called.\n");
...
{
	TRACE("CAcStCircle::GetAllFixes() called.\n");
...
{
	TRACE("CAcStCircle::GetPropertyDiffs() called.\n");
    ...
    try
    {
        ...
        for (long i=0; i < propCount; i++)
        {
			...
			csErrorValue.Format("index %d", errorPropValue.intVal);
            CString csFixValue;
			csFixValue.Format("index %d", fixPropValue.intVal);
...
{
	TRACE("CAcStCircle::FixError() called.\n");

	if (pError == NULL)
		return S_OK;

	SysFreeString(*pFailedReason);
	CString csFailedReason = "Fixed successfully.";
	...
	if (pFix == NULL)
	{
		...
		if (pFix == NULL)
		{
			...
			csFailedReason = "Can't find a recommended fix!";
	...
	try
	{
		...
		if (pCir == NULL)
			throw(Acad::ErrorStatus::eInvalidInput);
...
{
	TRACE("CAcStCircle::UpdateStatus() called.\n");
