...
{	
    ...
    gpDefContext1 = new defaultContext;   // default mode
...
//    menuLabel.LoadString(IDS_CMDARX_MENU_LABEL);
    ...
    // the arx app tries to add two menus. The second one is ignored
    // since each arx app is just allowed one. The arx app is identified
    // by the ID supplied here.
    b = acedAddDefaultContextMenu(gpDefContext1, pAppID);
	//assert(b == Adesk::kTrue);
...
void unloadApp()
{
    ...
    acedRemoveDefaultContextMenu(gpDefContext1);
