...
static Acad::ErrorStatus getEditOption(TCHAR* option,Adesk::Boolean& interrupted);
...
int& savedNumSides,AcGeVector3d&  savedNormal,TCHAR*    savedName , double& savedElevation);
...
polyeditCommand()
{
    ...
    while (!done && !interrupted) {
		...
		if ( getEditOption(option,interrupted) != Acad::eOk )
			continue;
        ...
        if (_tcscmp(option, _T("Grow")) == 0) {
            ...
            if (thickenPoly(poly, ent, savedCenter,savedStartPoint, savedNumSides,savedNormal,savedName,savedElevation) != Acad::eOk)
			{
...
getEditOption(TCHAR* option, Adesk::Boolean& interrupted)
{
    Acad::ErrorStatus es = Acad::eOk;
    int 	      as = RTNORM;
