...
//////////////////////////////////////////////////////////////////////////
// globals
extern int gRectHorizDynDimSetting;
extern int gRectVertDynDimSetting;
...
AcEdJig::DragStatus AcRectJig::sampler()
{
	...
	const TCHAR* keyWord = _T("Horz Vert All None");
...
AcRectJig::addDimData(int which, double dimScale)
{
	...
	if (which & kHorizontalDim) {
		...
		getDynDimSettings(dimData1, (AcDbDimData::DimDataFlags)gRectHorizDynDimSetting);
	...
	if (which & kVerticalDim) {
		...
		pAlnDim2->setDynamicDimension(true);
		...
		// check what the setting is for this dimension
		// this is configured using the SETDYNDIMMODES command 
		getDynDimSettings(dimData2, (AcDbDimData::DimDataFlags)gRectVertDynDimSetting);
