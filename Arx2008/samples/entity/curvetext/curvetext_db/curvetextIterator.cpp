...
CcurvetextIterator::CcurvetextIterator(const AsdkCurveText *pCurveText, AcGiWorldDraw *ctxt)
{
	...
	Acad::ErrorStatus es ;
	if ( (es =m_pCurveText->getEndParam(endParam)) != Acad::eOk )
	{
		_stprintf(errorBuffer, _T("!%s@%d: %s"), _T(__FILE__), __LINE__, MESSAGE(es));
		acutPrintf(errorBuffer);
		return ;
	}
	if ( (es =m_pCurveText->getDistAtParam(endParam, m_curveLength)) != Acad::eOk )
	{
		_stprintf(errorBuffer, _T("!%s@%d: %s"), _T(__FILE__), __LINE__, MESSAGE(es));
		acutPrintf(errorBuffer);
		return ;
	...
	//
	if ( (es =fromAcDbTextStyle(m_style, m_pCurveText->getStyle())) != Acad::eOk )
	{
		_stprintf(errorBuffer, _T("!%s@%d: %s"), _T(__FILE__), __LINE__, MESSAGE(es));
		acutPrintf(errorBuffer);
		return ;
