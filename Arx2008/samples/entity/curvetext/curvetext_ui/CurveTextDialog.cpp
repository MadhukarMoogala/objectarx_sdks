...
BEGIN_MESSAGE_MAP(CCurveTextDia, CAcUiDialog)
...
BOOL CCurveTextDia::OnInitDialog() 
{
	...
	if ( acdbHostApplicationServices()->workingDatabase()->getTextStyleTable(pStyleTable, AcDb::kForRead) != Acad::eOk )
		return TRUE ;

	AcDbTextStyleTableIterator* pIterator;
	if ( pStyleTable->newIterator(pIterator) != Acad::eOk )
		return TRUE ;
	...
	{
		if ( pIterator->getRecord(pStyle, AcDb::kForRead) != Acad::eOk )
			continue ;
	...
	if(curveTextIdArray.length() > 1)
	{
		...
		{
			m_editString.SetWindowText(_T("*Varies*"));
		}
		...
		{
			m_edit_size.SetWindowText(_T("*Varies*"));
		}
		if(global.isSpaceVarying)
		{
			m_editSpace.SetWindowText(_T("*Varies*"));
		}
