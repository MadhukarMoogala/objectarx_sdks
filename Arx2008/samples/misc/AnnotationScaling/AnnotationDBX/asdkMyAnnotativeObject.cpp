...
void asdkMyAnnotativeObject::myCommonDraw(AcGiCommonDraw* commonDraw, bool isWorldDraw)
{
        ...
        if (contextCollection) 
        {
          ...
          for (pIter->start(); !pIter->done(); pIter->next()) 
          {
            ...
            if (annotationScale && objectContextInterface->hasContext(this, *annotationScale))
            {
              ...
              if (fabs(getViewportScale(commonDraw, isWorldDraw) - scale) < 0.0001) 
              {
  ...
  else 
  {
    ...
    // now physically draw the for normal drawing case
    myDrawRoutine(commonDraw, isWorldDraw, getViewportScale(commonDraw, isWorldDraw));              
  }
}
...
double asdkMyAnnotativeObject::getViewportScale(AcGiCommonDraw *commonDraw, bool isWorldDraw)
{
  double scale = 1.0;
  // if worlddraw, then simply get the current annotation scale
  if (isWorldDraw)
  {
    // get the draw context database, then extract the current annotation scale
  ...
  else
  {
    ...
    if (viewport.openStatus() == Acad::eOk)
      viewport->annotationScale()->getScale(scale);
    // if in modelspace
    else
    {
      // get the draw context database, then extract the current annotation scale
...
//////////////////////////////////////////////////////////////////////////
Acad::ErrorStatus asdkMyAnnotativeObject::setMyTextHeight(double textHeight)
{
  ...
  mMyTextHeight = textHeight*scale;
