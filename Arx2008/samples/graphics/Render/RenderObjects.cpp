...
Acad::ErrorStatus CRenderObjects::addAPointLight()
{
	...
	AcGiShadowParameters2 shadowParams;
	shadowParams.setShadowsOn(true);
	shadowParams.setShadowType(AcGiShadowParameters::kAreaSampled);
	shadowParams.setShadowSamples(500);
	shadowParams.setShapeVisibility(true);
	shadowParams.setExtendedLightShape(AcGiShadowParameters2::kDisk);
	shadowParams.setExtendedLightRadius(1.0);
	
	es = addEntToCurrentSpace(pDb, pLight);
	if( Acad::eOk != es )
	{
		delete pLight;
		return es;
...
// Output some light information like physical intensity and lamp color type
void CRenderObjects::getLightInfo()
{
	Acad::ErrorStatus es = Acad::eOk;
	...
	if(RTNORM != acedEntSel(_T("\nSelect a light: "),entres,ptres))
		return;

	//	Get pointer to line
	if ( acdbGetObjectId (ltId, entres) != Acad::eOk )
		return;

	AcDbObjectPointer<AcDbLight> pLight(ltId,AcDb::kForRead);

	if(Acad::eOk == pLight.openStatus())
	{
		
		AcDbLight::PhysicalIntensityMethod pi = pLight->physicalIntensityMethod();
		if(AcDbLight::kPeakIntensity == pi)
			acutPrintf(_T("\nPhysical Intensity Method: %s"),_T("kPeakIntensity"));
		else if(AcDbLight::kFlux == pi)
			acutPrintf(_T("\nPhysical Intensity Method: %s"),_T("kFlux"));
		else if(AcDbLight::kIlluminance == pi)
			acutPrintf(_T("\nPhysical Intensity Method: %s"),_T("kIlluminance"));

		double phyInts = pLight->physicalIntensity();
		acutPrintf(_T("\nPhysical Intensity: %lf"),phyInts);

		double illumDist = pLight->illuminanceDistance();
		acutPrintf(_T("\nIlluminance Distance: %lf"),illumDist);

		AcDbLight::LampColorType lpColTyp = pLight->lampColorType();
		if(AcDbLight::kKelvin == pi)
			acutPrintf(_T("\nLamp Color Type: %s"),_T("kKelvin"));
		else if(AcDbLight::kPreset == pi)
			acutPrintf(_T("\nLamp Color Type: %s"),_T("kPreset"));
		
		double lpColTemp = pLight->lampColorTemp();
		acutPrintf(_T("\nLamp Color Temperature: %lf"),lpColTemp);

		AcDbLight::LampColorPreset lpColPres = pLight->lampColorPreset();
		acutPrintf(_T("\nLamp Color Presets: %d"),lpColPres);

		AcGiColorRGB rgbColor;
		es = pLight->lampColorRGB(rgbColor) ;
		acutPrintf(_T("\nLamp Color RGB: %lf, %lf, %lf"),rgbColor.red,rgbColor.green,rgbColor.blue);
	}

}

// Add a Sun with specific sky parameter settings
Acad::ErrorStatus CRenderObjects::addSunAndSky()
{
	Acad::ErrorStatus es = Acad::eOk;
	AcDbObjectPointer<AcDbSun> pSun;

	pSun.create();

	AcGiSkyParameters skyParams;

	skyParams.setAerialPerspective(true);
	skyParams.setIllumination(true);
	skyParams.setIntensityFactor(2.5);
	skyParams.setHaze(3.5);
	skyParams.setHorizonBlur(5.5);
	skyParams.setRedBlueShift(0.5);
	skyParams.setSaturation(1.0);
	AcCmColor grCol;
	grCol.setRGB(100,200,100);
	skyParams.setGroundColor(grCol);

	AcCmColor ntCol;

	ntCol.setRGB(20,20,20);
	skyParams.setNightColor(ntCol);
	...
	if( vt == RenderAPISample::ModelSpaceView )
	{
		AcDbObjectPointer<AcDbViewportTableRecord> pVTR(curViewId, AcDb::kForWrite); 
		if(pVTR.openStatus()==Acad::eOk) 
			pVTR->setSun(sunId, pSun.object());
	}
	else if( vt == RenderAPISample::PaperSpaceView )
	{
		AcDbObjectPointer<AcDbViewport> pV(curViewId, AcDb::kForWrite); 
		if(pV.openStatus()==Acad::eOk) 
			pV->setSun(sunId, pSun.object());
	}

	pSun->setSkyParameters(skyParams);

	return es;
}

// Add tone parameters for the current viewport
Acad::ErrorStatus CRenderObjects::setToneParams()
{
	Acad::ErrorStatus es = Acad::eOk;

	AcGiToneOperatorParameters toneParams;

	toneParams.setIsActive(true);
	toneParams.setBrightness(100);
	toneParams.setContrast(65);
	toneParams.setMidTones(9.5);
	toneParams.setExteriorDaylight(AcGiToneOperatorParameters::kDaylightOn);
	...
	if( vt == RenderAPISample::ModelSpaceView )
	{
		AcDbObjectPointer<AcDbViewportTableRecord> pVTR(curViewId, AcDb::kForWrite); 
		if(pVTR.openStatus()==Acad::eOk) 
			pVTR->setToneOperatorParameters(toneParams);
	}
	else if( vt == RenderAPISample::PaperSpaceView )
	{
		AcDbObjectPointer<AcDbViewport> pV(curViewId, AcDb::kForWrite); 
		if(pV.openStatus()==Acad::eOk) 
			pV->setToneOperatorParameters(toneParams);
	}
