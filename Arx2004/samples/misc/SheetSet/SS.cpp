		...
		const AcString& defSheetLoc)
{
	...
	if(!_access(fullPath.kTCharPtr(),0))
	{
	...
	if(isValid(templPath))
	{
		...
		AcString layoutName("TEST"); // Assume this layout exists in the DWT
		...
		const AcString& layoutDwgPath)
{
	...
	TCHAR sformat[32];
    _stprintf(sformat,_T("%d"),nSheetCount+1);
