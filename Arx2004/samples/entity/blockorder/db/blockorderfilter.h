...
//
#if !defined(_BLOCKORDERFILTER_H)
#define _BLOCKORDERFILTER_H
...
class DLLIMPEXP AsdkBlockOrderFilter : public AcDbFilter
{
    AcDbObjectIdArray mSentToTop;
    AcDbObjectIdArray mSentToBottom;
    static Adesk::Int16 classVersion;
public:
	ACRX_DECLARE_MEMBERS(AsdkBlockOrderFilter);
	virtual ~AsdkBlockOrderFilter();
    virtual AcRxClass* indexClass() const;
    virtual Acad::ErrorStatus dwgInFields (AcDbDwgFiler* pFiler);
    virtual Acad::ErrorStatus dwgOutFields(AcDbDwgFiler* pFiler) const;

    //BlockOrder protocol declared virtual 
    virtual const AcDbObjectIdArray& sentToTop()const {assertReadEnabled(); return mSentToTop;}
    virtual const AcDbObjectIdArray& sentToBottom()const {assertReadEnabled(); return mSentToBottom;}
