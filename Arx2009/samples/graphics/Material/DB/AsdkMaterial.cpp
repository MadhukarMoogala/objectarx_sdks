...
void AsdkMaterial::getMeshPts (AcGePoint3d meshPts[kMeshRows][kMeshCols]) const
{
    double scaleX = 1.0 / kRectCols;
    double scaleY = 1.0 / kRectRows;
    AcGeMatrix3d scaleMat;
         ...
         ++i, translateY += scaleY * 2.0) 
    {
             ...
             ++j, translateX += scaleX * 2.0) 
        {
            AcGeMatrix3d translateMat;
            translateMat.setToTranslation(AcGeVector3d(translateX, translateY, 0.0));
            meshPts[i][j] = (mTransform * translateMat * scaleMat) * AcGePoint3d(-1.0, -1.0, 0.0);
        }
    }
}
...
// Examine material assigned to this entity, if it meets our criteria for a 
// landscape material, return true
bool AsdkMaterial::isLandscape(void) 
{
    // PERFORMANCE NOTE: Querying the material frequently like this is slow and
    // not entirely reliable.  In real practice, this flag should be passed in 
    // explicitly to AsdkMaterial::setMaterialId(), and then persisted in the 
    // DWG/DXF.  This weaker method is used here to avoid changing the file 
    // format of AsdkMaterial.
    //
    bool bIsLandscape = false;

    AcDbMaterial *pMatObj = NULL;
    Acad::ErrorStatus status = acdbOpenObject(pMatObj, mMaterialId, AcDb::kForRead);
    if (status == Acad::eOk) 
    {
        // Check for global illumination off, final gather off, diffuse, opacity, 
        // and normal maps on to decide if this is a landscape material or not.
        if (bIsLandscape = (pMatObj->globalIllumination() == AcGiMaterialTraits2::kGlobalIlluminationNone
                            && pMatObj->finalGather() == AcGiMaterialTraits2::kFinalGatherNone))
        {
            ...
            pMatObj->diffuse(color, map);
            if (bIsLandscape = (map.texture() != NULL)) 
            {
                pMatObj->opacity(factor, map);
                if (bIsLandscape = (map.texture() != NULL))
                {
                    pMatObj->normalMap(map, method, factor);
                    bIsLandscape = (map.texture() != NULL);
                }
            }
        }
...
{
    Adesk::UInt32 vpDrawFlags = isLandscape() ? AcGiDrawable::kDrawableViewDependentViewportDraw : 0;
    return AcDbEntity::setAttributes(pTraits) | vpDrawFlags;
}
...
Adesk::Boolean AsdkMaterial::worldDraw (AcGiWorldDraw *pWorldDraw)
{
    ...
    if (isLandscape())
        return Adesk::kFalse; // Viewport draw
    ...
    return Adesk::kTrue;  // No viewport draw
}

void AsdkMaterial::viewportDraw (AcGiViewportDraw* pVpDraw)
{
    // Draw the color and material
    pVpDraw->subEntityTraits().setColor(1);
    pVpDraw->subEntityTraits().setMaterial(mMaterialId);

    // Get the mesh points
    AcGePoint3d meshPts[kMeshRows][kMeshCols];
    getMeshPts(meshPts);

    // Create and set preset texture coordinates into mesh
    AcGePoint3d mappingCoords[kMeshRows][kMeshCols];
    int i, j;
    for (i = 0; i < kMeshRows; ++i)
        for (j = 0; j < kMeshCols; ++j)
            mappingCoords[i][j] = AcGePoint3d(i / (double)(kMeshCols - 1),
                                              j / (double)(kMeshRows - 1),
                                              0.0);
    AcGiVertexData vertexData;
    vertexData.setMappingCoords(AcGiVertexData::kAllChannels,
                                (AcGePoint3d *)mappingCoords);

    // Draw front faces only
    pVpDraw->subEntityTraits().setDrawFlags(AcGiSubEntityTraits::kDrawFrontfacesOnly);

    // Draw the mesh
    pVpDraw->geometry().mesh(kMeshRows, kMeshCols, 
                             (const AcGePoint3d *)meshPts,
                             NULL,
                             NULL,
                             &vertexData);
}
