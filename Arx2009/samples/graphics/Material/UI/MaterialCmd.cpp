...
{ 
    ...
    _T("MATFILTER"),    // Toggles the material's filter setting
};
...
void MaterialCmd::addTileCmd(void) 
{ 
    AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase();
    matCmds[pDb].invokeCmd(kAddTile);
}

void MaterialCmd::addWaveCmd(void) 
{ 
    AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase();
    matCmds[pDb].invokeCmd(kAddWave);
}

void MaterialCmd::addSpeckleCmd(void) 
{ 
    AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase();
    matCmds[pDb].invokeCmd(kAddSpeckle);
}

void MaterialCmd::addCheckerCmd(void) 
{ 
    AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase();
    matCmds[pDb].invokeCmd(kAddChecker);
}

void MaterialCmd::modifyFilterCmd(void)
{
    AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase();
    matCmds[pDb].invokeCmd(kModifyFilter);
}
    ...
    case AcGiVariant::kColor:
        {
                ...
                color.colorMethod() == AcCmEntityColor::kByColor)
            {
                ...
                {
                    const int strLength = keylength < 300 ? 301
                                                          : (keylength + 1);
                    colorKey = (TCHAR *)malloc((strLength) * sizeof(TCHAR));
                    color.getDictionaryKey(colorKey, strLength);
                }
            }
...
void MaterialCmd::modifyMaterialFilter(TSTDSTRING &name)
{
    // Open the material dictionary for reading
    AcDbDictionary *pMaterialDict;
    Acad::ErrorStatus dbStatus;
        ...
        ->getMaterialDictionary(pMaterialDict, AcDb::kForRead) == Acad::eOk)
    {
        ...
        // Toggle the diffuse texture filter setting
        if (dbStatus == Acad::eOk) {
            AcGiMaterialMap  map;
            AcGiMaterialColor color;
            pMaterialObj->diffuse(color, map);
            ...
            switch (map.filter())
            {
            case AcGiMaterialMap::kFilterDefault:  
                map.setFilter(AcGiMaterialMap::kFilterNone);  
                break;
            default:
                assert(false);
            case AcGiMaterialMap::kFilterNone:  
                map.setFilter(AcGiMaterialMap::kFilterDefault);  
                break;
            }
