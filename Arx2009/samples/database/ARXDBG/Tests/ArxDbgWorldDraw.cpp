...
ACRX_NO_CONS_DEFINE_MEMBERS(ArxDbgGiWorldGeometry, AcGiWorldGeometry2)
                 ...
                 bool                       isPie ) const
{
    printGeomMsg(_T("--> ellipticalArc(center,normal,majorAxisLength,")
        _T("minorAxisLength,startDegreeInRads,endDegreeInRads,tiltDegreeInRads,isPie)"));
    return Adesk::kFalse;
}

Adesk::Boolean 
ArxDbgGiWorldGeometry::symbol  (
                 AcGiSymbolType type,
...
{
    printGeomMsg(_T("--> grip (nbPoints,pPoints,borderColor,boxColor)"));
    return Adesk::kFalse;
}
