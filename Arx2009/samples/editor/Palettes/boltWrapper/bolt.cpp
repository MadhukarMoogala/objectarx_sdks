...
STDMETHODIMP CBolt::put_materialName(BSTR newVal)
{
    USES_CONVERSION;
    BEGIN_PROP_GETPUT(asdkBolt,AcDb::kForWrite)
        _tcscpy_s(pEntity->materialName,_countof(pEntity->materialName),OLE2T(newVal));
...
STDMETHODIMP CBolt::put_partNumber(BSTR newVal)
{
    USES_CONVERSION;
    BEGIN_PROP_GETPUT(asdkBolt,AcDb::kForWrite)
        _tcscpy_s(pEntity->partNumber,_countof(pEntity->partNumber),OLE2T(newVal));
