...
STDMETHODIMP CBoltTool::put_MaterialName(BSTR newVal)
{
    ...
    _tcscpy_s(m_MaterialName, _countof(m_MaterialName),szMaterialName);
    return S_OK;
}
...
STDMETHODIMP CBoltTool::put_PartNumber(BSTR newVal)
{
    ...
    _tcscpy_s(m_PartNumber, _countof(m_PartNumber),szPartNumber);
    return S_OK;
}
...
STDMETHODIMP CBoltTool::dropCallback(AcDbEntity *pDropEntity)
{
    ...
    if(pBolt)
    {
        ...
        _tcscpy_s(m_szLayer, _countof(m_szLayer),pBolt->layer());
        ...
        _tcscpy_s(m_MaterialName, _countof(m_MaterialName),pBolt->materialName);
        _tcscpy_s(m_PartNumber, _countof(m_PartNumber),pBolt->partNumber);
...
STDMETHODIMP CBoltTool::New()
{
    ...
    _tcscpy_s(m_szLayer, _countof(m_szLayer),_T("0"));
    _tcscpy_s(m_MaterialName, _countof(m_MaterialName),_T("Stainless"));
    _tcscpy_s(m_PartNumber, _countof(m_PartNumber),_T("Unassigned"));
                                    ...
                                    /* [retval][out] */ DWORD *pFlag)
{
    ...
    else if (idCmd == m_nMenuIds[1])
    {
        ...
        ASSERT(pPalette != NULL);
        New();
        _tcscpy_s(m_MaterialName, _countof(m_MaterialName),_T("Aluminum"));
        _tcscpy_s(m_PartNumber, _countof(m_PartNumber),_T("Unassigned"));
