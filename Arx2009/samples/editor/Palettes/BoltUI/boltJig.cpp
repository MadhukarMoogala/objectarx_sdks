...
void asdkCreateTools()
{
    try
    {
    	// Check if a BoltPalette is already added and return  
		TCHAR catlogName[127], paletteName[127]; 
		int nCatalog = (int)AcTcGetManager()->GetCatalogCount(); 
		for(int i = 0 ; i < nCatalog ; i ++) 
		{ 
			AcTcCatalogItem *pCatlogItem = AcTcGetManager()->GetCatalog(i); 
			pCatlogItem->GetName(catlogName,120); 

			if (_tcsicmp(catlogName, _T("BoltCatalog")) == 0)
			{
				int nChildren = (int)pCatlogItem->GetChildCount(); 
				for(int j = 0 ; j < nChildren; j++ ) 
				{ 
					AcTcCatalogItem *pCatlogPalette = pCatlogItem->GetChild(j); 
					pCatlogPalette->GetName(paletteName, 120); 

					if (_tcsicmp(paletteName, _T("BoltPalette")) == 0)
						return;
				} 
			}
		}
        ...
        if(pShapePackage->GetChildCount()==0)
        {
            ...
            _tcscpy_s(tool.m_MaterialName,_countof(tool.m_MaterialName),_T("Aluminum"));
            _tcscpy_s(tool.m_PartNumber,_countof(tool.m_PartNumber),_T("Unassigned"));			
            ...
            _tcscpy_s(tool.m_MaterialName,_countof(tool.m_MaterialName),_T("Galvanized"));
            _tcscpy_s(tool.m_PartNumber,_countof(tool.m_PartNumber),_T("Unassigned"));			
