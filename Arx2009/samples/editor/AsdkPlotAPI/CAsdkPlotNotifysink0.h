...
public:
    void SendPlotEvent(LPCTSTR EventString)
    {
        ...
        TCHAR sbuff[32];
        _itot_s(GetCurrentProcessId(),sbuff,_countof(sbuff),10);
        
        TCHAR tszDest[512];
        _tcscpy_s(tszDest,_countof(tszDest),_T("ID:"));
        _tcscat_s(tszDest,_countof(tszDest),sbuff);
        _tcscat_s(tszDest,_countof(tszDest),_T("  "));
        _tcscat_s(tszDest,_countof(tszDest),EventString);
