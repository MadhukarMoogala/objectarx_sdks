...
Acad::ErrorStatus AsdkRings::dwgOutFields (AcDbDwgFiler *pFiler) const {
	...
	if ( (es =pFiler ->writeInt16 ((Adesk::Int16)AsdkRings::kCurrentVersionNumber)) != Acad::eOk )
		return (es) ;
...
Acad::ErrorStatus AsdkRings::dwgInFields (AcDbDwgFiler *pFiler) {
	...
	if ( (es =pFiler->readInt16(&version)) != Acad::eOk )
		return (es) ;
	if ( version > (Adesk::Int16) AsdkRings::kCurrentVersionNumber ){
...
AsdkRings::dxfInFields(AcDbDxfFiler *filer)
{
    ...
    //
    if( !filer->atSubclassData( _T("AsdkRings") )) {
...
AsdkRings::dxfOutFields(AcDbDxfFiler *filer) const
{
    ...
    filer->writeItem( AcDb::kDxfSubclass, _T("AsdkRings") );
...
Acad::ErrorStatus 
AsdkRings::getGsMarkersAtSubentPath( const AcDbFullSubentPath & subPath, AcArray<Adesk::GsMarker> & gsMarkers ) const
{
