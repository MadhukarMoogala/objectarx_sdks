...
class CSelectionFilterUIApp : public AcRxArxApp {
	...
	static const TCHAR * dbxname; // _T("asdkSelectionFilterDb.dbx")
...
public:
	...
	virtual AcRx::AppRetCode On_kInitAppMsg (void *pkt) {
        ...
        if ( !::acrxAppIsLoaded( dbxname ) ) {
            if ( !acrxLoadModule(dbxname, false, true) )
                return (AcRx::kRetError);
		}
		//- bump the reference count 
		acrxLoadModule(dbxname, false, false);
...
const TCHAR * CSelectionFilterUIApp::dbxname = _T("asdkSelectionFilterDb.dbx");
