...
void InitApplication()
{
	...
	//{{AFX_ARX_INIT
	AddCommand(_T("DWFAPI"), _T("dwfapi"), _T("DWFAPI"), ACRX_CMD_TRANSPARENT | ACRX_CMD_USEPICKSET,DWFAPI);
	//}}AFX_ARX_INIT
...
void UnloadApplication()
{
	// NOTE: DO NOT edit the following lines.
	//{{AFX_ARX_EXIT
	acedRegCmds->removeGroup(_T("ASDKPROPERTY_INS"));
	//}}AFX_ARX_EXIT
...
void myPrintf(CString text, CString filename)
{
	...
	{
		AcFILE stream;
		if((stream.fopen(filename, _T("a+"))) == NULL)
		{
...
void TdDMMReactor::OnBeginEntity(AcDMMEntityReactorInfo * pInfo) //#4
{
	if (A1 == false)
	{
		...
		myPrintf(_T("\n------------------------------------\n"), OUTFILE);
        pInfo->GetPlotLogger()->logMessage(_T("\nTdDMMReactor::OnBeginEntity"));
		A1 = true;
	}
	if (g_bIncludeBlockInfo)
	{
		...
		if (pEntity->isKindOf(CSteelSection::desc()))
		{
			...
			if(_tcscmp(OLE2T(prop00->GetName()), _T("Test Name")) == 0)
			...
			if(_tcscmp(OLE2T(prop00->GetValue()), _T("Test Value")) == 0)
			...
			if(_tcscmp(OLE2T(prop00->GetType()), _T("Test Type")) == 0)
			...
			if(_tcscmp(OLE2T(prop00->GetUnits()), _T("Test Unit")) == 0)
			...
			if(_tcscmp(OLE2T(props00.GetId()), _T("Partha ID")) == 0)
            ...
            if(_tcscmp(OLE2T(props00.GetNamespaceUrl()), _T("http:\\www.autodesk.com")) == 0)
			...
			if(_tcscmp(OLE2T(props00.GetNamespaceLocation()), _T("Partha Location")) == 0)
		...
		if (pEntity->isKindOf(AcDbBlockReference::desc()) && (g_pDxServices != NULL)) // block
		{
			...
			if(_tcscmp(OLE2T(node1->nodeName()), _T("PARTHA02")) == 0)
...
void TdDMMReactor::OnBeginSheet(AcDMMSheetReactorInfo * pInfo) //#3
{
	...
	if (!pInfo->isModelLayout()) // Layout
	{
		...
		m_cstr.Format(_T("%g"),drawingScale);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),PlotWinMinX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),PlotWinMinY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),PlotWinMaxX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),PlotWinMaxY);
		m_wchar = T2W(m_cstr);
		...
		if (_tcscmp(ViewPlotted, _T("")) == 0)
			propViewPlotted.SetValue(L"View name not defined.");
		else
		{
			m_cstr.Format(_T("%s"),ViewPlotted);
			m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),displayMinX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),displayMinY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),displayMaxX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),displayMaxY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),layoutMarginMinX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),layoutMarginMinY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),layoutMarginMaxX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),layoutMarginMaxY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),printableBoundsX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),printableBoundsY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),stepsPerInch);
		m_wchar = T2W(m_cstr);
		propMaxBoundsY.SetValue(m_wchar);
		propMaxBoundsY.SetCategory(L"ParthaSheetProp");
		propVec.push_back(propMaxBoundsY);
		...
		if (_tcscmp(configuration, _T("")) == 0)
			propConfiguration.SetValue(L"Configuration name not defined.");
		else
		{
			m_cstr.Format(_T("%s"),configuration);
			m_wchar = T2W(m_cstr);
		...
		if (_tcscmp(plotToFilePath, _T("")) == 0)
			propPlotToFilePath.SetValue(L"Plot To File Path not defined.");
		else
		{
			m_cstr.Format(_T("%s"),plotToFilePath);
			m_wchar = T2W(m_cstr);
		...
		if (_tcscmp(plotToFileName, _T("")) == 0)
			propPlotToFileName.SetValue(L"Plot To File Name not defined.");
		else
		{
			m_cstr.Format(_T("%s"),plotToFileName);
			m_wchar = T2W(m_cstr);
		...
		if (_tcscmp(caninicalMediaName, _T("")) == 0)
			propCaninicalMediaName.SetValue(L"Caninical Media Name not defined.");
		else
		{
			m_cstr.Format(_T("%s"),caninicalMediaName);
			m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),plotBoundsMinX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),plotBoundsMinY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),plotBoundsMaxX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),plotBoundsMaxY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),layoutBoundsMinX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),layoutBoundsMinY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),layoutBoundsMaxX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),layoutBoundsMaxY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),effectivePlotOffsetX);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%g"),effectivePlotOffsetY);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%d"),effectivePlotOffsetXdevice);
		m_wchar = T2W(m_cstr);
		...
		m_cstr.Format(_T("%d"),effectivePlotOffsetYdevice);
		m_wchar = T2W(m_cstr);
	...
	else // model Layout
	{
		...
		USES_CONVERSION;
		// SetRole()
		TCHAR* role = _T("Partha's role");
		res.SetRole(T2W(role));
		const TCHAR* outRole = OLE2T(res.GetRole());
		if (_tcscmp(outRole, role) == 0)
		...
		// SetMime()
		TCHAR* mime = _T("Partha's mime");
		res.SetMime(T2W(mime));
		const TCHAR* outMime = OLE2T(res.GetMime());
		if (_tcscmp(outMime, mime) == 0)
		...
		// SetPath()
        TCHAR* path = _T("C:\\Partha's Path");
		res.SetPath(T2W(path));
		const TCHAR* outPath = OLE2T(res.GetPath());
		if (_tcscmp(outPath, path) == 0)
		...
		const TCHAR* outPath1 = OLE2T(res1.GetPath());
		if (_tcscmp(outPath1, path) == 0)
		...
		const TCHAR* outPath2 = OLE2T(res2.GetPath());
		if (_tcscmp(outPath2, path) == 0)
...
void TdPublishReactor::OnAboutToBeginPublishing(AcPublishBeginJobInfo* pInfo) //#1
{
	...
	if (!acrxServiceIsRegistered(_T("AcEPlotX"))) {
		acrxLoadModule(_T("AcEPlotx.arx"), false, false);
	}
	HINSTANCE hInst = NULL;
	hInst = ::GetModuleHandle(_T("AcEPlotX.arx"));
	ASSERT(NULL != hInst);
	if ((hInst)) {
	...
	valuePairVec = pInfo->GetPrivateData(_T("AutoCAD Block Data"));    
	...
	if (_tcscmp(nameValuePair.name(), _T("Partha_JobBegin_Private_Name")) == 0)
...
void TdPublishReactor::OnAboutToEndPublishing(AcPublishReactorInfo *pInfo) //#9
{
	...
	if (NULL != g_pDMMReactor) {
		HINSTANCE hInst0 = ::GetModuleHandle(_T("AcEPlotX.arx"));
		if ((hInst0)) {
			ACGLOBREMOVEDMMREACTOR pAcGlobalRemoveDMMReactor = (ACGLOBREMOVEDMMREACTOR)GetProcAddress(hInst0, _T("AcGlobRemoveDMMReactor"));
	...
	// dwfFileName()
	const TCHAR* fileName = pInfo->dwfFileName();
	...
	// tempDwfFileName()
	const TCHAR* tempFileName = pInfo->tempDwfFileName();
	if (_tcscmp(tempFileName, _T("")) == 0)
	...
	// dwfPassword()
	const TCHAR* dwfPassword = pInfo->dwfPassword();
	if (_tcscmp(dwfPassword, _T("")) == 0)
	...
	AcArray<TCHAR*>* urSectionArray;
	AcArray<TCHAR*>* urDataArray;
...
void TdPublishReactor::OnBeginPublishingSheet(AcPublishSheetInfo *pInfo) //#2
{
	...
	if (pEntity != NULL)
		if ((_tcscmp(pEntity->dwgName(), _T("")) != 0) &&
			(_tcscmp(pEntity->layout(), _T("")) != 0))
	...
	// GetUniqueId()
	const TCHAR* uniqueId = pInfo->GetUniqueId();
	if (_tcscmp(uniqueId, _T("")) == 0)
...
void TdPublishUIReactor::OnInitPublishOptionsDialog(IUnknown **pUnk, AcPublishUIReactorInfo *pInfo)
{
	...
	for (int i = 0; i < size; i++) {
		...
		name = pPair.name();
		value = pPair.value();
        str = _T("\nPassed: Name-Value  of the private 2nd private data written is ") + name + _T(",") + value;
		myPrintf(str, OUTFILE);
	}
