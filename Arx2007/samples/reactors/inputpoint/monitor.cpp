          ...
          const TCHAR*                         tooltipString)
{
    ...
    //
#define HISTORY_CHECK(x)  if (history & Acad:: ## x) acutPrintf(_T("%s "), _T(#x))
...
#define OSMASK_CHECK(x)  if (osnapMasks & AcDb:: ## x) acutPrintf(_T("%s "), _T(#x))
...
#define OSMASK_CHECK(x)  if (osnapOverrides & AcDb:: ## x) acutPrintf(_T("%s "), _T(#x))
...
#undef OSMASK_CHECK
    ...
    if ( 0 != customOsnapOverrides.length() )
    {
        ...
        for ( int x = 0; x < customOsnapOverrides.length(); x++)
            acutPrintf( _T("%s "), customOsnapOverrides[x]->globalModeString() );
        acutPrintf( _T("\n") );
    }

    // Show object snap or forced pick information: the entities in the aperture
    acutPrintf( _T("%d apertured entities, "), apertureEntities.length() );
    ...
    //
    acutPrintf( _T("%d key point entities, "), keyPointEntities.length() );
    ...
    {
        additionalTooltipString = _T(", Additional comment!");
