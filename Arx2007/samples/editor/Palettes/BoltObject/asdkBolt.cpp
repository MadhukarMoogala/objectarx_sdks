...
asdkBolt::asdkBolt()
{
	...
	shaftDiameter=3.0f;
	calculatePnts();
	_tcscpy(materialName,_T("Stainless"));
	_tcscpy(partNumber,_T("unassigned"));
}
...
Acad::ErrorStatus asdkBolt::dwgInFields(AcDbDwgFiler* pFiler)
{
	...
	case (1):
		...
		_tcscpy(partNumber,pString);
		pFiler->readString(&pString);
		_tcscpy(materialName,pString);
...
void asdkBolt::drawThread(AcGiWorldDraw* wD)
{
	...
	{
		assertWriteEnabled();
		_tcscpy(materialName,mN);
	}

	void asdkBolt::getMaterialName(TCHAR *mN)
	{
		assertReadEnabled();
		_tcscpy(mN,materialName);
	}
	...
	{
		assertWriteEnabled();
		_tcscpy(partNumber,pN);
	}

	void asdkBolt::getPartNumber(TCHAR *pN)
	{
		assertReadEnabled();
		_tcscpy(pN,partNumber);
	}
