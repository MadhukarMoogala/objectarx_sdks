...
//-----------------------------------------------------------------------------
//----- drawordersamp.cpp : Initialization functions
//-----------------------------------------------------------------------------
#include "StdAfx.h"
...
AC_IMPLEMENT_EXTENSION_MODULE(drawordersampDLL)
...
#ifdef NEVER
AFX_EXTENSION_MODULE drawordersampExtDLL ={ NULL, NULL } ;
#endif
...
BOOL WINAPI DllMain (HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved) {
	...
	if ( dwReason == DLL_PROCESS_ATTACH ) {
		...
		drawordersampDLL.AttachInstance (hInstance) ;
		InitAcUiDLL () ;
	} else if ( dwReason == DLL_PROCESS_DETACH ) {
		drawordersampDLL.DetachInstance () ;
	}
	return (TRUE) ;
}
