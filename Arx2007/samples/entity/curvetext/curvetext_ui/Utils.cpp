...
getDefaultCharacterSize()
{
	...
	acedGetVar(_T("VIEWSIZE"), &ViewSize);
...
AcDbEntity*
selectEntity(TCHAR * msg, AcDb::OpenMode openMode)
{
    AcDbObjectId eId;
...
AcDbEntity*
selectEntity(AcDbObjectId& eId, TCHAR * msg, AcDb::OpenMode openMode)
{
    ads_name en;
    ads_point pt;
    ...
    if ( acdbGetObjectId(eId, en) )
		return NULL;
    AcDbEntity * ent;
	if ( acdbOpenObject(ent, eId, openMode) != Acad::eOk )
		return NULL;
    return ent;
}
...
getCurvesSelection(AcDbObjectIdArray& curvesArray)
{
	...
	//
	int ret = acedSSGet(_T("_I"), NULL, NULL, NULL, ss); 
	Acad::ErrorStatus es;
	if(ret != RTNORM)
	{
		// if not, create one
		...
		{
			acutPrintf(_T("No valid curves selected.\n"));
			return es;
		}
		if(es != Acad::eOk)
			return es;
...
{
	acutPrintf(_T("X=%.*lf Y=%.*lf Z=%.*lf"), _prec, pt.x,
								 		  _prec, pt.y,
										  _prec, pt.z);
}
...
getTextStyleName(CString& styleName, AcDbObjectId styleId)
{
	...
	AcDbObject *pObj;
	Acad::ErrorStatus es ;
	if ( (es =acdbOpenAcDbObject(pObj, styleId, AcDb::kForRead)) != Acad::eOk )
	{
		TCHAR errorBuffer[256];
		_stprintf(errorBuffer, _T("!%s@%d: %s"), _T(__FILE__), __LINE__, acadErrorStatusText(es));
...
getTextStyleId(const CString styleName)
{
	...
	if ( pDb->getTextStyleTable(pTsTable, AcDb::kForRead) != Acad::eOk )
		return AcDbObjectId::kNull;

	AcDbObjectId styleId;
	if ( pTsTable->getAt(styleName, styleId) != Acad::eOk )
		return AcDbObjectId::kNull;
	pTsTable->close();
	return styleId;
}
...
GLOBALS::GLOBALS () {
	...
	styleId = getTextStyleId(_T("STANDARD"));
	// curve is shown
	showCurve = TRUE;
	// text is shown
	showText = TRUE;
...
void updateRegistry()
{
	...
	AcadAppInfo appInfo;
	appInfo.setAppName(_T("CurveTextARX")); // Application Name
	appInfo.setModuleName(acedGetAppName()); // ARX module location.
	appInfo.setAppDesc(_T("CurveTextARX"));	 // Description
	...
	appInfo.writeGroupNameToRegistry(_T("CURVETEXT")); // GroupName
	appInfo.writeCommandNameToRegistry(_T("ACDG_CT"),_T("CURVETEXT")); // Commands...
	appInfo.writeCommandNameToRegistry(_T("ACDG_REVERSE"),_T("REVERSE"));// --
}
