...
HRESULT CTransmittalGraphDialog::traverseTransmittalDependents (HTREEITEM treeRoot, ITransmittalFile *graphRoot)
{
	...
	for (int i=0; i<numOfNodes; ++i) 
	{
		...
		HTREEITEM childNode = m_treeGraph.InsertItem((char *)_bstr_t(sourcePath), treeRoot, treeRoot);

		// lets find out what type of extension it has
		_splitpath((char *)_bstr_t(sourcePath), drive, dir, fname, ext);
		// drawing files
		if (!strcmpi (ext, _T(".dwg")))
		{
			// set the bitmap for this filetype
			m_treeGraph.SetItemImage (childNode, 2, 2);
		}
		// shape fonts
		else if (!strcmpi (ext, _T(".shx")))
		{
			// set the bitmap for this filetype
			m_treeGraph.SetItemImage (childNode, 3, 3);
		}
		// true type fonts
		else if (!strcmpi (ext, _T(".ttf")))
		{
			// set the bitmap for this filetype
			m_treeGraph.SetItemImage (childNode, 4, 4);
		}
		// postscript fonts
		else if (!strcmpi (ext, _T(".pfb")))
		{
			// set the bitmap for this filetype
			m_treeGraph.SetItemImage (childNode, 5, 5);
		}
		// lisp
		else if (!strcmpi (ext, _T(".fmp")) || !strcmpi (ext, _T(".lsp")))
		{
			// set the bitmap for this filetype
