...
ArxDbgGiSubEntityTraits::setLineType(const AcDbObjectId linetypeId)
{
    ...
    str.Format(_T("--> setLineType() = %s"),
                            static_cast<const char*>(ltypeName));
    printSubentMsg(str);
}
...
void
ArxDbgGiSubEntityTraits::setSelectionMarker(const Adesk::Int32 markerId)
{
    CString str;
    str.Format(_T("--> setSelectionMarker() = %ld"),
                        static_cast<long>(markerId));
    printSubentMsg(str);
}
