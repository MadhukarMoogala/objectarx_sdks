...
AecUiPrEntitySet::go()
{
    ...
    m_ss.setKeywordCallback(_DNT("FIlter _ FIlter"), AecUiPrEntitySet::keywordCallback);
                ...
                if (es == Acad::eOk) {
                    if (layer->isLocked()) {
                        ...
                        if (es == Acad::eOk) {
                            ...
                            AEC_ASSERT(0);
                            m_lockedLayerNames.Add(_DNT(""));   // have to keep balanced
                        }
                    }
...
AecUiPrEntitySet::buildSelSetFilter()
{
    ...
    if (m_classTypeCache.isEmpty() == Adesk::kFalse) {
        tmpRb = acutNewRb(-4);
        newString(_DNT("<or"), tmpRb->resval.rstring);
    ...
    if (len > 0) {
        tmpRb = acutNewRb(-4);
        newString(_DNT("<not"), tmpRb->resval.rstring);
        rbList.addTail(tmpRb);
...
struct resbuf*
AecUiPrEntitySet::keywordCallback(const char* str)
{
