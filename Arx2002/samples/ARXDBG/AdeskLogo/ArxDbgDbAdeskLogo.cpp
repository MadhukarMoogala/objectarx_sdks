...
Adesk::Boolean
ArxDbgDbAdeskLogo::worldDraw(AcGiWorldDraw* worldDraw)
{
...
/****************************************************************************
**
**  ArxDbgDbAdeskLogo::viewportDraw
**
**  **jma
**
*************************************/
...
void
ArxDbgDbAdeskLogo::viewportDraw(AcGiViewportDraw* vportDraw)
{
...
ArxDbgDbAdeskLogo::drawLabel(AcGiCommonDraw* drawContext, ArxDbgDbAdeskLogoStyle* lStyle)
{
    ...
    if (labelStr.IsEmpty())
        ...
        // signatures.
    AcDbMText* tmpMtext = new AcDbMText;
    tmpMtext->setLocation(AcGePoint3d(1.25, 0.0, 0.0)); // set to be just to the right of the logo
    tmpMtext->setTextHeight(1.0);                       // unit-block concept where scaling will adjust height
    tmpMtext->setWidth(0.0);
    tmpMtext->setContents(labelStr);
    tmpMtext->setAttachment(AcDbMText::kBottomLeft);

    AcDbDatabase* db = database();
    if (db == NULL)
        db = acdbHostApplicationServices()->workingDatabase();

    tmpMtext->setDatabaseDefaults(db);

    if (lStyle && (lStyle->textStyleId() != AcDbObjectId::kNull))
        tmpMtext->setTextStyle(lStyle->textStyleId());

    drawContext->rawGeometry()->draw(tmpMtext);
...
Acad::ErrorStatus
ArxDbgDbAdeskLogo::transformBy(const AcGeMatrix3d& xform)
{
    assertWriteEnabled();
	...
	return ArxDbgDbEntity::transformBy(xform);
}
