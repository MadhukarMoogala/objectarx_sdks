...
ArxDbgCmdAdeskLogo::styleAdd()
{
    ...
    if (acdbSNValid(prName.value(), false) != RTNORM) {
        acedAlert("Invalid symbol name!");
        return;
    }

    bool doSolidFill = true;
    if (ArxDbgUtils::yesNoPromptDef("Use solid fill ", doSolidFill, true) != ArxDbgUiPrBase::kOk)
        return;
...
ArxDbgCmdAdeskLogo::styleModify()
{
    ...
    bool doSolidFill = true;
    if (ArxDbgUtils::yesNoPromptDef("Use solid fill ", doSolidFill, logoStyle->isSolidFill()) != ArxDbgUiPrBase::kOk) {
		logoStyle->close();
        return;
	}
