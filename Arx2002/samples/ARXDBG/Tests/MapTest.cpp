...
MapTestReactor::endInsert(AcDbDatabase* pTo) 
{
    ...
    for (int i=0 ; i<len; i++) {
		...
		if (es == Acad::eOk) {
			printObj("Swap from", ent);
			printObj("Swap to", tmpNode->m_newExtId);
...
mapTestImportDwg()
{
	...
	Acad::ErrorStatus es;
	es = ArxDbgUtils::getFileNameForRead("Drawing To Import", NULL, "dwg", mapReactor->m_dwgName, FALSE, FALSE);
	if (es != Acad::eOk)
		return;
