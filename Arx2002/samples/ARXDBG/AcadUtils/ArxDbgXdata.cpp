...
ArxDbgAppXdata::findItemStart(int tag)
{
    ...
    while (tmp) {
        ...
        if (tmp->restype == 1002) {
            if (!strcmp(tmp->resval.rstring, AcadString::openBrace))
                nestedLists++;
            else if(!strcmp(tmp->resval.rstring, AcadString::endBrace))
                nestedLists--;
            tmp = tmp->rbnext;
        }
...
void
ArxDbgAppXdata::setGenericString(int tag, const char* value, int dxfType)
{
    resbuf* itemPtr;
...
void
ArxDbgAppXdata::setLayer(int tag, const char* value)
{
...
{
    return (rb->restype == 1002) && (!strcmp(rb->resval.rstring, AcadString::openBrace));
}
...
{
    return (rb->restype == 1002) && (!strcmp(rb->resval.rstring, AcadString::endBrace));
}
...
void
ArxDbgXdataList::appendString(const char* value)
{
...
void
ArxDbgXdataList::appendLayer(const char* value)
{
...
bool
ArxDbgXdataListIterator::setGenericString(const char* value, int dxfType)
{
    if (m_curPtr && (m_curPtr->restype == dxfType)) {
...
bool
ArxDbgXdataListIterator::setLayer(const char* value)
{
    bool isValid = ArxDbgUtils::symbolExists(AcDbLayerTableRecord::desc(), value,
					acdbHostApplicationServices()->workingDatabase());
    ASSERT(isValid == true);
