...
// Object Version
#define VERSION 1
#define kClassName "AdcgCurveText"

ACRX_DXF_DEFINE_MEMBERS(AdcgCurveText, AcDbCurve,  
						AcDb::kDHL_CURRENT, 
...
AdcgCurveText::AdcgCurveText(AcDbCurve *pCurve)
{
	m_normal	=	AcGeVector3d(0.0, 0.0, 1.0);
	m_string	=	"";
 	m_showCurve	=	true;                        
...
AdcgCurveText::AdcgCurveText(AcDbCurve *pCurve, const AdcgCurveText *pCurveText)
{
...
Adesk::Boolean
AdcgCurveText::worldDraw(AcGiWorldDraw* wd)
{
...
void
AdcgCurveText::saveAs(AcGiWorldDraw* wd, AcDb::SaveType st )
{
	// if m_space is 0, then force it to 1 !
	if(!m_space)
		m_space = 1.0;

	// see comment in AdcgCurveText::worldDraw about
	// AcGiFiler
	//
	AcDbVoidPtrArray ptrArray;
...
Acad::ErrorStatus
AdcgCurveText::transformBy(const AcGeMatrix3d& xform)
{
	assertWriteEnabled();
...
Acad::ErrorStatus
AdcgCurveText::dxfInFields(AcDbDxfFiler* filer)
{
	assertWriteEnabled();
...
Acad::ErrorStatus
AdcgCurveText::dxfOutFields(AcDbDxfFiler* filer) const
{
    assertReadEnabled();
...
Acad::ErrorStatus
AdcgCurveText::dwgInFields(AcDbDwgFiler* filer)
{
	assertWriteEnabled();
...
Acad::ErrorStatus
AdcgCurveText::dwgOutFields(AcDbDwgFiler* filer) const
{
	assertReadEnabled();
...
Acad::ErrorStatus
AdcgCurveText::getGeomExtents(AcDbExtents& extents) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus 
AdcgCurveText::explode(AcDbVoidPtrArray& entitySet) const
{
	assertReadEnabled();
	Acad::ErrorStatus es;
	assert(m_pCurve);
...
Acad::ErrorStatus 
AdcgCurveText::getGripPoints(AcGePoint3dArray& gripPoints,
...
Acad::ErrorStatus
AdcgCurveText::getOsnapPoints(AcDb::OsnapMode     osnapMode,
							  int                 gsSelectionMark,
...
Acad::ErrorStatus
AdcgCurveText::getGsMarkersAtSubentPath(const AcDbFullSubentPath& subPath,
...
Acad::ErrorStatus
AdcgCurveText::getSubentPathsAtGsMarker(AcDb::SubentType     type,
									     int                 gsMark,
										 const AcGePoint3d&  pickPoint,
										 const AcGeMatrix3d& viewXform,
										 int&                numPaths,
...
Acad::ErrorStatus 
AdcgCurveText::getStretchPoints(AcGePoint3dArray& stretchPoints) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::moveGripPointsAt(const AcDbIntArray& indices, const AcGeVector3d& offset)
{
	assertWriteEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::moveStretchPointsAt(const AcDbIntArray& indices, const AcGeVector3d& offset)
{
	assertWriteEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus 
AdcgCurveText::boundingBoxIntersectWith(const AcDbEntity* pEnt,
...
Acad::ErrorStatus
AdcgCurveText::intersectWith(const AcDbEntity* pEnt,
							 AcDb::Intersect   intType,
							 const AcGePlane&  projPlane,
							 AcGePoint3dArray& points,
							 int               thisGsMarker,
...
AcDbEntity* 
AdcgCurveText::subentPtr(const AcDbFullSubentPath& path) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus 
AdcgCurveText::getTransformedCopy(const AcGeMatrix3d& xform,
									 AcDbEntity*&  pEnt) const
{
	...
	if(es != Acad::eOk)
		return es;
	AdcgCurveText *pCurveText = new AdcgCurveText(AcDbCurve::cast(pEnt), this);
...
void
AdcgCurveText::getEcs(AcGeMatrix3d& retVal) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Adesk::Boolean
AdcgCurveText::isPeriodic() const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getPlane(AcGePlane& plane, AcDb::Planarity& planarity) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getStartParam(double& param) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getEndParam(double& param) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus 
AdcgCurveText::getStartPoint(AcGePoint3d& point) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getEndPoint(AcGePoint3d& point) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getPointAtParam(double param, AcGePoint3d& point) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getParamAtPoint(const AcGePoint3d& point, double& param) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getDistAtParam(double param, double& dist) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getParamAtDist(double dist , double& param) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getDistAtPoint(const AcGePoint3d& point, double& dist) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getPointAtDist(double dist, AcGePoint3d& point) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getFirstDeriv(double param, AcGeVector3d& vect) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getFirstDeriv(const AcGePoint3d& point, AcGeVector3d& vect)const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getSecondDeriv(double dist, AcGeVector3d& vect) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getSecondDeriv(const AcGePoint3d& point, AcGeVector3d& vect) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus
AdcgCurveText::getClosestPointTo(const AcGePoint3d& pt, AcGePoint3d& ptres,
...
Acad::ErrorStatus
AdcgCurveText::getClosestPointTo(const AcGePoint3d& pt, const AcGeVector3d& dir,
...
Acad::ErrorStatus
AdcgCurveText::getOrthoProjectedCurve(const AcGePlane& plane, AcDbCurve*& curve) const
{
	assertReadEnabled();
	Acad::ErrorStatus es;
	assert(m_pCurve);
	...
	pCurveText = new AdcgCurveText(curve, this);
...
Acad::ErrorStatus
AdcgCurveText::getProjectedCurve(const AcGePlane& plane, const AcGeVector3d& dir,
...
Acad::ErrorStatus
AdcgCurveText::getOffsetCurves(double offset, AcDbVoidPtrArray& offsetCurves) const
{
	assertReadEnabled();
	Acad::ErrorStatus es;
	assert(m_pCurve);
	...
	for(int i=0; i<length; i++)
	{
		...
		if(pOffsetCurve == NULL)
			continue;
		pCurveText = new AdcgCurveText(pOffsetCurve, this);
...
Acad::ErrorStatus
AdcgCurveText::getSpline(AcDbSpline*& spline) const
{
	assertReadEnabled();
	Acad::ErrorStatus es;
	assert(m_pCurve);
	...
	AdcgCurveText* pCurveText;
	pCurveText = new AdcgCurveText( AcDbCurve::cast(spline), this);
...
Acad::ErrorStatus
AdcgCurveText::getSplitCurves(const AcGeDoubleArray& darray,
							  AcDbVoidPtrArray& curveSegments) const
{
	...
	for(int i =0; i<length; i++)
	{
		pCurveText = new AdcgCurveText( AcDbCurve::cast((AcRxObject *)curveSegments[i]), this);
...
Acad::ErrorStatus
AdcgCurveText::getSplitCurves(const AcGePoint3dArray& parray,
							  AcDbVoidPtrArray& curveSegments) const
{
	assertReadEnabled();
	Acad::ErrorStatus es;
...
Acad::ErrorStatus 
AdcgCurveText::extend(double newParam)
{
	assertWriteEnabled();
	assert(m_pCurve);
...
Acad::ErrorStatus 
AdcgCurveText::extend(Adesk::Boolean extendStart,
...
Acad::ErrorStatus 
AdcgCurveText::getArea(double& area) const
{
	assertReadEnabled();
	assert(m_pCurve);
...
void 
AdcgCurveText::setCurvePtr(AcDbCurve* pCurve)
{
	assertWriteEnabled();
	assert(pCurve != NULL);
	m_pCurve = pCurve;
...
Acad::ErrorStatus
AdcgCurveText::setLineWeight(AcDb::LineWeight newVal, Adesk::Boolean doSubents)
{
	assertWriteEnabled();
	if(m_pCurve != NULL)
...
Acad::ErrorStatus
AdcgCurveText::setLayer(AcDbObjectId newVal, Adesk::Boolean doSubents)
{
	assertWriteEnabled();
	if(m_pCurve != NULL)
...
Acad::ErrorStatus
AdcgCurveText::setLayer(const char* newVal, Adesk::Boolean doSubents)
{
	assertWriteEnabled();
	if(m_pCurve != NULL)
		AOK(m_pCurve->setLayer(newVal, doSubents));
...
Acad::ErrorStatus
AdcgCurveText::setLinetype(AcDbObjectId newVal, Adesk::Boolean doSubents)
{
	assertWriteEnabled();
	if(m_pCurve != NULL)
...
Acad::ErrorStatus
AdcgCurveText::setLinetype(const char* newVal, Adesk::Boolean doSubents)
{
	assertWriteEnabled();
	if(m_pCurve != NULL)
		AOK(m_pCurve->setLinetype(newVal, doSubents));
...
Acad::ErrorStatus
AdcgCurveText::setColor(const AcCmColor& color, Adesk::Boolean doSubents)
{
	assertWriteEnabled();
	if(m_pCurve != NULL)
...
Acad::ErrorStatus
AdcgCurveText::setLinetypeScale(const double ltscale, Adesk::Boolean doSubents)
{
	assertWriteEnabled();
	if(m_pCurve != NULL)
...
void
AdcgCurveText::setStyle(CString string)
{
	assertWriteEnabled();
...
Acad::ErrorStatus
AdcgCurveText::getStyle(CString styleName) const
{
	assertReadEnabled();
...
Adesk::Boolean
AdcgCurveText::AcGiFiler(AcGiWorldDraw* wd, AcDb::SaveType st, AcDbVoidPtrArray &entitySet) const
{
	assertReadEnabled();

	// Checks if m_pCurve is of type AdcgCurveText.
	// in which case we'll take special precautions.
	//
	assert(m_pCurve);
	AdcgCurveText *pct = AdcgCurveText::cast(m_pCurve);
	if(NULL != pct)
	{
