...
void AsdkCommandReactor::commandEnded(const char*)
{
    recordCommandDuration();
}

void AsdkCommandReactor::commandCancelled(const char*)
{
    recordCommandDuration();
}

void AsdkCommandReactor::commandFailed(const char*)
{
    recordCommandDuration();
}
...
int cmdcount_findfile (const char *fname, char *result)
{
    ...
    acdbFree (tmpResult.resval.rstring) ;
#else 
...
Adesk::Boolean updateCumulativeStats()
{
    ...
    if (cmdcount_findfile(statFileName, statFilePath) != RTNORM) {
        ...
        if (cmdcount_findfile(getProgramFiles(/*NOXLATE*/"nt.cfg"), statFilePath) == RTNORM) {
            // Found it. Truncate the acadnt.cfg and add in our filename
            statFilePath[strlen(statFilePath)-strlen(getProgramFiles(/*NOXLATE*/"nt.cfg"))] = EOS;
#ifndef OEM 
        } else if (cmdcount_findfile(getProgramFiles(/*NOXLATE*/".cfg"), statFilePath) == RTNORM) {
            // Found it. Truncate the acad.cfg and add in our filename
            statFilePath[strlen(statFilePath)-strlen(getProgramFiles(/*NOXLATE*/".cfg"))] = EOS;
#endif // OEM 
        } else if (acedFindFile(getProgramFiles(/*NOXLATE*/".exe"), statFilePath) == RTNORM) {
            // Found it.  Truncate the acad.exe, and add in our filename
            statFilePath[strlen(statFilePath)-strlen(getProgramFiles(/*NOXLATE*/".exe"))] = EOS;
        } else {
...
void selfReg()
{
	...
	char regpath[1024];
	char loaderKey[1024];
	wsprintf(regpath, "%s\\AutodeskApps", acrxProductKey());
	wsprintf(loaderKey, "%s\\%s\\Loader", regpath, gAppSubKey);
	//  skip if already present and OverWrite is not true
	if (*RegGetString(HKEY_LOCAL_MACHINE, loaderKey, "MODULE"))
		return;

	int r = (AcadApp::kOnAutoCADStartup | AcadApp::kOnCommandInvocation | AcadApp::kOnLoadRequest);
	acrxRegisterApp((AcadApp::LoadReasons)r, gAppSubKey, regpath, 2);

	//write command names
	char buf[1024];
	wsprintf(buf, "%s\\%s\\%s", regpath, gAppSubKey, "Commands");
	RegPutString(HKEY_LOCAL_MACHINE, buf, "CMDSTAT", "CMDSTAT");
	RegPutString(HKEY_LOCAL_MACHINE, buf, "CMDCOUNT", "CMDCOUNT");

	//write the module and name keys
	wsprintf(buf, "%s\\%s\\%s", regpath, gAppSubKey, "Loader");
	RegPutString(HKEY_LOCAL_MACHINE, buf, "MODULE", gAppFileName);

    wsprintf(buf, "%s\\%s\\%s", regpath, gAppSubKey, "Name");
    RegPutString(HKEY_LOCAL_MACHINE, buf, gAppSubKey, gAppSubKey);
}
